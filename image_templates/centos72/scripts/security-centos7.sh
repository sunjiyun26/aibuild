...
systemAuthConf=/etc/security/pwquality.conf
sed -i  "s/.*minlen.*=.*\d*/minlen=8/g" $systemAuthConf
sed -i  "s/.*minclass.*=.*\d*/minclass=3/g" $systemAuthConf
...
serviceStatus2 firewalld.service
statusFlag=$?
if [ $statusFlag -eq 1 ]; then
  echo 'service firewalld.service has been installed and running'
else
  systemctl start firewalld.service
  systemctl enable firewalld.service
  echo 'firewalld.service status '$(serviceStatus2 firewalld.service)
fi

# $(service firewalld.service save)
# sleep 3
